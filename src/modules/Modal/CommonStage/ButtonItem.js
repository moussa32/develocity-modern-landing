...
const ButtonItem = ({ selected, handleSelect, image, mainText, secondaryText, itemToSelect , disabled}) => {
  const handleClick = (event) => {
    handleSelect(itemToSelect);
  };

  return (
    <div className={`d-flex button-item-wrapper ${selected === mainText && "active"} ${disabled ? "disabledbtn" :"" }` } onClick={handleClick}>
